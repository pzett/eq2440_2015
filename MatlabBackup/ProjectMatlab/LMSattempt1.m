...
for n=1:M,
	...
	% Generate Y. Set elements of Y that does not exist to zero
    Y=zeros(N,1);
    Y(1:min(N,n),1)=flip(rec2(max(1,n-N+1):n));
    
    
	% Estimate of x
    xhat2(n,1)=thetahat2*Y;
    
    
	% Update the n+1 row in the matrix thetahat which in the notation in the Lecture Notes
	% corresponds to thetahat(n)

	thetahat2=thetahat2+(muu*Y*(rec1(n)-xhat2(n,1))).';
end
toc
%%
shatLMS=z-xhat;
...
%rec1downsample = downsample(rec1,2);
%rec2downsample = downsample(rec1(2:),2);
