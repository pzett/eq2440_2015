...
public class StudentCode extends StudentCodeBase {
	...
	{ 
		...
		useSensors =  SOUND_IN;//|SOUND_OUT;// CAMERA;//CAMERA_RGB;//WIFI_SCAN | SOUND_OUT; //GYROSCOPE;//SOUND_IN|SOUND_OUT;//WIFI_SCAN | ACCELEROMETER | MAGNETIC_FIELD | PROXIMITY | LIGHT;//TIME_SYNC|SOUND_IN;//TIME_SYNC | ACCELEROMETER | MAGNETIC_FIELD | PROXIMITY | LIGHT | SOUND_IN;						
		...
		init_done=true;
		buffer=new short[2048]; // 1024 samples sent to codec at a time
	...
	public void start()
	{	
		gamma1p5 = Gamma.gamma(1.5);
		spectralDistance = new double[windowLength];
	...
	public void accelerometer(long time, double x, double y, double z)
	{
		...
		triggerTime = "A: "+format4_2.format(x)+":"+format4_2.format(y)+":"+format4_2.format(z-9.2) + " The Time:" + time;
	}
	...
	// SIGNAL PROCESSING FUNCTIONS IN HERE
	public double[] HammingWindow(double[] signal_in)
	{
		double[] hammingWindowedSignal = new double[windowLength+1];
	    for (int i = 0; i < windowLength; i++)
	    {
	        hammingWindowedSignal[i] = (double) (signal_in[i] *  (0.54 - 0.46*Math.cos(2.0 * Math.PI * i / windowLength)));
	    }
	    return hammingWindowedSignal;
	}

	public boolean voiseActivityDetector(double[] signalIn,double[] noiseIn){
	...
	public double[] framing(double[] signalIn){
		
		
		
		
		return choppedSignal;
	}
	
	
	public double[] logMMSESTSA(double[] signalIn){
	...
	public double[] overLappAndAdd(double[] signalIn){
	...
	public void filterInitializations() {
  ...
  long outSampleCounter=0;
	...
	public void playsoundexample(){
		...
		if (file_loaded) {
			...
			long currentBufferTime =  frameWork.getstartBufferStartRecordTime() + 1000000L*outSampleCounter/this.sampleRate;
			outSampleCounter += buffer.length;
			p_streaming_buffer_out(buffer,buffer.length,"N3");
			sound_out(buffer,buffer.length); // Send buffer to player	
			if ((this.loggingOn) & (this.logSound)) 
			{
				String s = 	null;
				s = "O;"+System.currentTimeMillis()+";"+buffer.length+";"+Arrays.toString(buffer)+"\n";
				try {
					if(frameWork.getlogFile() != null)
						frameWork.getlogFile().write(s.getBytes(),0,s.length());
