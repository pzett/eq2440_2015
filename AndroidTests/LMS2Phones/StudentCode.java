...
public class StudentCode extends StudentCodeBase {
	...
	{ 
		...
		noise_filename="drillinglong.txt";
		//noise_filename="drillingshort.txt";
		//noise_filename="drillingontable.txt";
	}
	...
	{	
		...
		in.open(noise_filename);    	  
		int no_of_real=in.readInt();
		noise=new short[no_of_real];
		for(int i=0; i<noise.length; i++){
			noise[i]=(short) in.readDouble(); 
		};
		thetahat=new double[orderLMS];
		first=true;
		counter=0;
		bigBuffer=new ArrayList<short[]>();
	}
	...
	{ 
		...
		if(this.myGroupID==1 && counter>3){
			sound_out(bigBuffer.get(0),bigBuffer.get(0).length);
			bigBuffer.remove(0);
			counter=4;
		}
	...
	{			
		if (myGroupID==0){
			p_streaming_buffer_out(samples, length, messageGroups[1]);
		}
	}
	...
	public void streaming_buffer_in(short[] buffer, int length, int senderId)
	{
		if (myGroupID==1){
			...
			short[] noiseEstimate;
			noiseEstimate=lmsXaviShort(buffer,noise,orderLMS, mu);
			for (i=length;--i>=0;){
				buffer[i]-=noiseEstimate[i];
			}
