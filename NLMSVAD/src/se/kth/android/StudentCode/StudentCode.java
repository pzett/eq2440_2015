...
//				}
			...
			case EMPTY:
				...
				set_output_text(messageData);
				while (senderBuffer.size()>4+Math.abs(diff) && noiseBuffer.size()>4+Math.abs(diff)){
			...
			case PLAY:
				...
				messageData="Here are a few interesting parameters:"+"\n"+
						"mu="+mu+"  //  "+"mu'="+modifiedmu+"\n"+
						"diff="+diff+"  //  "+"delay="+delay+"  //  "+"total delay="+totalDelay+"\n"+
						"noise cancellation="+noiseCancellation+"  //  "+"VD="+VD+"\n"+
	...
	public void stringFromUser(String user_input){		
		orderLMS=Integer.parseInt(user_input);
		backToWAITING=true;
	}
