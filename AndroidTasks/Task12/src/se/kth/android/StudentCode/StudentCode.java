...
public class StudentCode extends StudentCodeBase {
	...
	{ 
		...
		useSensors =  CAMERA;// CAMERA;//CAMERA_RGB;//WIFI_SCAN | SOUND_OUT; //GYROSCOPE;//SOUND_IN|SOUND_OUT;//WIFI_SCAN | ACCELEROMETER | MAGNETIC_FIELD | PROXIMITY | LIGHT;//TIME_SYNC|SOUND_IN;//TIME_SYNC | ACCELEROMETER | MAGNETIC_FIELD | PROXIMITY | LIGHT | SOUND_IN;
		...
		introText = "This is the best version of FrameWork";
	...
	{ 
		...
		long difference=current_time-start_time;
		int interval=(int)difference%4000;
		plotG=(0<=interval && interval<2000);
	...
	{		
		...
		if (hasstarted){
			green.setTextSize(200);
			plotCanvas.rotate(-90,width/2,height/2);
			if(plotG || plotBoth){
				plotCanvas.drawText("G", 100, 400, green);
			}
			if(!plotG || plotBoth){
				plotCanvas.drawText("H", 100, 400, green);
			}
			plotCanvas.rotate(90,width/2,height/2);
		}
	}
