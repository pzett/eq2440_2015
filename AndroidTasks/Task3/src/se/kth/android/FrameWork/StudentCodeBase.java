...
Runnable() {
        ...
        if (true){
            Thread p_ipSoundListenerThread0 = new Thread(new Runnable() {
                synchronized public void run()
                {
                    p_listenBufferSocket(5000);
                }});

            p_ipSoundListenerThread0.start();
        	        	
            Thread p_ipSoundListenerThread1 = new Thread(new Runnable() {
                synchronized public void run()
                {
                    p_listenBufferSocket(5001);
                }});

            p_ipSoundListenerThread1.start();

            Thread p_ipSoundListenerThread2 = new Thread(new Runnable() {
                synchronized public void run()
                {
                    p_listenBufferSocket(5002);
                }});

            p_ipSoundListenerThread2.start();
	...
	void p_listenBufferSocket(int port_num){
		...
		try {
			...
			while(true)
			{
				...
				int groupId=-1;
				String s=(256+ip[0])+"."+(256+ip[1])+"."+ip[2]+"."+ip[3];
				...
				streaming_buffer_in(buffer, length, groupId);
			}
...
//			receive.close();
	...
	boolean[] p_connected={false,false,false};	
	
	InetSocketAddress p_clientAddr[]={null,null,null};
	Socket p_client[]={null,null,null};
	DataOutputStream p_dos[]={null,null,null}; //=new DataOutputStream[max_num_groups];
	
	
	public void p_streaming_buffer_out(short[] buffer, int length, String groupIdString)
	{
		int groupId = -1;
		int i = 0;
		for(String g:messageGroups)
		...
		if ((!p_connected[groupId]) && (myGroupID>-1) && (groupId>-1)){
			...
			if(pm != null && pm.ip != null && !pm.ip.equals(getLocalIpAddress()) && pm.groupId == groupId)
			{
				...
				p_clientAddr[groupId]= new InetSocketAddress(pm.ip,5000+myGroupID);
				p_client[groupId] = new Socket();
				try {
					p_client[groupId].connect(p_clientAddr[groupId]);
					p_dos[groupId] = new DataOutputStream(p_client[groupId].getOutputStream());
					p_connected[groupId]=true;
					
				} catch (IOException e) {
					// Remove unconnectable
					//ips.remove(ip);
					
					p_clientAddr[groupId]=null;
					p_client[groupId]=null;
					p_dos[groupId]=null;
					p_connected[groupId]=false;
		 ...
		 if (p_connected[groupId]){
		 try {
           p_dos[groupId].writeInt(length);
           for(int idx=0;idx<length;idx++)
        	 p_dos[groupId].writeShort(buffer[idx]);          
